...
const transportInfo = [
  { icon: Car, title: 'By Car / Two-Wheeler', desc: 'Free parking is available within the Shivbalak Market complex adjacent to the clinic. Entry from Faizabad Road.' },
  { icon: Bus, title: 'By Bus / Auto', desc: 'Alight at Tiwariganj Bus Stop on Faizabad Road. The clinic is a 2-minute walk from the stop, opposite the Mahindra Showroom.' },
  { icon: Train, title: 'By Metro / Rail', desc: 'Nearest metro station is Polytechnic (Lucknow Metro – Blue Line), approx. 1.5 km from the clinic. Auto-rickshaws available.' },
]

export default function ContactPage() {
  const [ref, inView] = useInView()
  const [submitted, setSubmitted] = useState(false)
  const [form, setForm] = useState({ name: '', phone: '', date: '', department: '' })
    ...
    <>
      ...
      <div className="bg-navy pt-28 pb-16 relative overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto px-6 text-center">
          ...
          <h1 className="font-serif text-4xl md:text-5xl text-white leading-tight mb-4">Contact <span className="text-gold-gradient">&amp; Appointments</span></h1>
      ...
      <section ref={ref} className="py-24 bg-white">
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid lg:grid-cols-2 gap-12">
            {/* Left: Location */}
            <motion.div initial={{ opacity: 0, x: -30 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.7, delay: 0.1 }} className="space-y-5">
              {/* Info cards */}
              <div className="bg-clinical-white rounded-2xl p-6 border border-gray-100 shadow-sm space-y-5">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="font-sans text-navy text-sm font-medium leading-relaxed">
                      Shivbalak Market, Opp. Mahindra Showroom,<br />Tiwariganj, Faizabad Road, Lucknow
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <a href="tel:+919369643922" className="block font-sans text-navy text-sm font-medium hover:text-gold transition-colors">+91 9369643922</a>
                    <a href="tel:+918858580214" className="block font-sans text-navy text-sm font-medium hover:text-gold transition-colors">+91 8858580214</a>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="font-sans text-gray-400 text-xs uppercase tracking-wide mb-0.5">OPD Hours</p>
                    <p className="font-sans text-navy text-sm font-medium">Mon–Sat: 9:00 AM – 7:00 PM</p>
                    <p className="font-sans text-gray-400 text-xs mt-0.5">Emergency available 24/7 on call</p>
                  </div>
                </div>
            ...
            <motion.div initial={{ opacity: 0, x: 30 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.7, delay: 0.25 }}>
              {submitted ? (
                <div className="flex flex-col items-center justify-center h-full text-center p-12 bg-clinical-white rounded-2xl border border-gray-100 shadow-sm">
                  ...
                  <p className="font-sans text-gray-500 text-sm leading-relaxed">Our team will call you within one business hour to confirm your appointment.</p>
                </div>
              ) : (
                <form onSubmit={handleSubmit} className="bg-clinical-white rounded-2xl p-8 border border-gray-100 shadow-sm space-y-5">
                  <div>
                    <h3 className="font-serif text-2xl text-navy mb-1">Book an Appointment</h3>
                    <p className="font-sans text-gray-400 text-xs mb-2">Fill the form — we will call you back promptly.</p>
                  </div>
                  ...
                  {[
                    ...
                    <div key={id}>
                      <label className="block font-sans text-gray-600 text-xs mb-1.5 font-medium uppercase tracking-wide">{label}</label>
                      <input
                        type={type}
                  ...
                  <div>
                    <label className="block font-sans text-gray-600 text-xs mb-1.5 font-medium uppercase tracking-wide">Preferred Date</label>
                    <input
                      type="date"
                      value={form.date}
                  ...
                  <div>
                    <label className="block font-sans text-gray-600 text-xs mb-1.5 font-medium uppercase tracking-wide">Department</label>
                    <select
                  ...
                  <button type="submit" className="w-full py-4 gold-gradient text-navy font-sans font-bold text-sm rounded-xl hover:opacity-90 transition-opacity shadow-lg shadow-gold/25 tracking-wide">
      ...
      <section className="py-20 bg-clinical-white">
        <div className="max-w-7xl mx-auto px-6">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.7 }} className="text-center mb-12">
            <p className="text-gold tracking-[0.3em] text-xs font-sans uppercase mb-4">Getting Here</p>
            <h2 className="font-serif text-3xl md:text-4xl text-navy leading-tight mb-4">
              Transport <span className="text-gold-gradient">&amp; Accessibility</span>
            </h2>
          ...
          <div className="grid sm:grid-cols-3 gap-6">
            {transportInfo.map((t, i) => (
              ...
              >
                ...
                <p className="font-sans text-gray-500 text-sm leading-relaxed">{t.desc}</p>
              </motion.div>
            ))}
          </div>
        </div>
